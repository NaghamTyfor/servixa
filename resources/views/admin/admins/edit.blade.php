...
@section('styles')
    ...
    <style>
        ...
        .info-card-title svg {
            width: 20px; height: 20px; stroke: var(--primary-purple);
        }
        .info-list {
    ...
    <div class="modern-grid">
        ...
        <div class="form-card">
            <div class="form-card-header">
                ...
                <h3>{{ __('admin.edit_admin_details') }}</h3>
            </div>
            <div class="form-card-body">
                <form action="{{ route('admin.admins.update', $admin) }}" method="POST" id="editAdminForm">
                    @csrf
                    @method('PUT')

                    {{-- حقل الاسم مع مؤشر التغيير --}}
                    <div class="input-group-modern">
                        <label class="input-label" for="name">
                            ...
                            {{ __('admin.name') }}
                            <span id="nameChangeIndicator" class="change-indicator" style="display:none;"></span>
                        </label>
                    ...
                    <div class="input-group-modern">
                        <label class="input-label" for="email">
                            ...
                            {{ __('admin.email') }}
                            <span id="emailChangeIndicator" class="change-indicator" style="display:none;"></span>
                        </label>
                        <div class="input-wrapper">
                            <span class="input-icon">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="input-group-modern">
                        ...
                        <div class="input-wrapper password-wrapper">
                            ...
                            <input type="password"
                                   ...
                                   id="password">
                            <button type="button" class="password-toggle" onclick="togglePassword('password')" aria-label="{{ __('admin.toggle_password') }}">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="input-group-modern">
                        ...
                        <div class="input-wrapper password-wrapper">
                            ...
                            <input type="password"
                                   ...
                                   id="passwordConfirm">
                            <button type="button" class="password-toggle" onclick="togglePassword('passwordConfirm')" aria-label="{{ __('admin.toggle_password') }}">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
                                    <circle cx="12" cy="12" r="3"/>
                                </svg>
                    ...
                    <div class="roles-card">
                        <div class="roles-card-header">
                            ...
                            <span id="rolesChangeIndicator" class="change-indicator" style="display:none;"></span>
                        </div>
        ...
        <div class="sidebar-modern">
            ...
            <div class="live-preview-card">
                ...
                <div class="preview-avatar" id="previewAvatar">{{ strtoupper(substr($admin->name, 0, 1)) }}</div>
                <div class="preview-item">
                    ...
                    <div class="preview-item-content">
                        ...
                        <div class="preview-item-value" id="previewName">{{ $admin->name }}</div>
                    </div>
                </div>
                <div class="preview-item">
                    <div class="preview-item-icon">
                    ...
                    <div class="preview-item-content">
                        ...
                        <div class="preview-item-value" id="previewEmail">{{ $admin->email }}</div>
                    </div>
                </div>
                <div class="preview-item">
                    <div class="preview-item-icon">
                    ...
                    <div class="preview-item-content">
                        ...
                        <div class="preview-item-value" id="previewRoles">
            ...
            <div class="info-card">
                ...
                <ul class="info-list">
                    ...
                    <li>
                        ...
                        <strong>{{ __('admin.created_at') }}:</strong> {{ $admin->created_at->format('Y-m-d') }}
                    </li>
                    <li>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <circle cx="12" cy="12" r="10"/>
...
<script src="{{ asset('plugins/src/sweetalerts2/custom-swealert.js') }}"></script>
<script>
    // عناصر الصفحة
    ...
    const rolesIndicator = document.getElementById('rolesChangeIndicator');
    let selectedRoles = []; // مصفوفة لحفظ الأدوار المختارة

    // الأدوار الأصلية (للتعديل)
    const originalRoles = @json($admin->roles->pluck('name'));
    ...
    // تهيئة الأدوار المختارة بناءً على الأدوار الحالية للمسؤول
    @foreach($admin->roles as $role)
        addRole('{{ $role->name }}');
    @endforeach

    // تحديث مؤشرات التغيير للأسم والبريد
    ...
    function updateNameEmailIndicators() {
        const nameChanged = nameInput.value.trim() !== originalName;
        nameInput.classList.toggle('is-changed', nameChanged);
        nameIndicator.style.display = nameChanged ? 'inline-block' : 'none';

        const emailChanged = emailInput.value.trim() !== originalEmail;
        emailInput.classList.toggle('is-changed', emailChanged);
        emailIndicator.style.display = emailChanged ? 'inline-block' : 'none';
    }

    nameInput.addEventListener('input', function() {
        updatePreview();
        updateNameEmailIndicators();
    });
    ...
    document.getElementById('editAdminForm').addEventListener('submit', function(e) {
        // إذا لم تكن هناك أدوار محددة، نضمن إرسال مصفوفة فارغة
        if (selectedRoles.length === 0) {
            // إضافة حقل مخفي فارغ
            const input = document.createElement('input');
            input.type = 'hidden';
        ...
        btn.disabled = true;
        btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span> {{ __('admin.updating') }}';
    });
