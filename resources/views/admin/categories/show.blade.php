...
@section('styles')
    ...
    <style>
        :root {
            --primary-purple: #7c3aed; --purple-light: #a78bfa; --purple-dark: #5b21b6;
            --purple-soft: #ede9fe; --purple-glow: rgba(124,58,237,0.22);
            --card-radius: 24px; --transition: 0.3s cubic-bezier(0.4,0,0.2,1);
        }
        ...
        .tabs-nav { display:flex;gap:0.5rem;background:#fafafe;border:1px solid #ede9fe;border-radius:20px;padding:0.4rem; }
        body.dark .tabs-nav { background:rgba(255,255,255,0.03);border-color:rgba(124,58,237,0.2); }
        .tab-btn { display:flex;align-items:center;gap:0.5rem;padding:0.6rem 1.2rem;border-radius:14px;border:none;background:transparent;color:#64748b;font-weight:600;font-size:0.88rem;cursor:pointer;transition:all var(--transition); }
        body.dark .tab-btn { color:#94a3b8; }
        .tab-btn.active { background:linear-gradient(135deg,var(--primary-purple),var(--purple-dark));color:white;box-shadow:0 4px 12px var(--purple-glow); }
        .tab-count { background:rgba(255,255,255,0.25);border-radius:30px;padding:0.1rem 0.6rem;font-size:0.75rem;font-weight:700; }
        .tab-btn:not(.active) .tab-count { background:var(--purple-soft);color:var(--primary-purple); }
        body.dark .tab-btn:not(.active) .tab-count { background:rgba(124,58,237,0.15);color:var(--purple-light); }
        ...
        .modern-table tbody tr { animation:rowIn 0.4s ease both;transition:background 0.15s; }
        ...
        .tbl-btn-view { background:#eef2ff;color:#4f46e5; }
        .tbl-btn-view:hover { background:#4f46e5;color:white; }
        ...
        body.dark .empty-icon { background:rgba(255,255,255,0.05); }
...
<div class="show-wrapper">
    ...
    <div class="page-header-modern">
        ...
        <div class="action-buttons">
            @can('categories.edit')
            <a href="{{ route('admin.categories.edit', $category) }}" class="btn-action btn-action-primary">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                {{ __('admin.edit') }}
            </a>
            @endcan
            <a href="{{ route('admin.categories.sub-categories.index', $category) }}" class="btn-action btn-action-primary">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 16 12 14 15 10 15 8 12 2 12"/><path d="M5.45 5.11L2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg>
                {{ __('admin.sub_categories') }}
            </a>
            @can('categories.delete')
            <button type="button" class="btn-action btn-action-danger" id="deleteBtn"
                    data-type-name="{{ $category->getTranslation('name', app()->getLocale()) }}"
        ...
        <div class="stats-grid">
            <div class="stat-card" style="animation-delay:0.1s">
                ...
                <div class="stat-content"><div class="stat-label">{{ __('admin.sub_categories') }}</div><div class="stat-value">{{ $category->subCategories->count() }}</div></div>
            </div>
            <div class="stat-card" style="animation-delay:0.15s">
                <div class="stat-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg></div>
                <div class="stat-content"><div class="stat-label">{{ __('admin.dynamic_fields') }}</div><div class="stat-value">{{ $category->dynamicFields->count() }}</div></div>
            </div>
            <div class="stat-card" style="animation-delay:0.2s">
                ...
                <div class="stat-content"><div class="stat-label">{{ __('admin.created_at') }}</div><div class="stat-value" style="font-size:1rem;">{{ $category->created_at->format('Y-m-d') }}</div></div>
            </div>
        </div>
    </div>
    ...
    <div class="modern-grid">
        <div>
            ...
            <div class="tabs-container">
                <div class="tabs-nav">
                    <button class="tab-btn active" data-tab="sub-categories">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 16 12 14 15 10 15 8 12 2 12"/><path d="M5.45 5.11L2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg>
                        {{ __('admin.sub_categories') }}
                        <span class="tab-count">{{ $category->subCategories->count() }}</span>
                    </button>
                    <button class="tab-btn" data-tab="dynamic-fields">
                        ...
                        <span class="tab-count">{{ $category->dynamicFields->count() }}</span>
                    </button>
                </div>
            </div>

            {{-- Sub-Categories Tab --}}
            <div class="tab-pane active" id="tab-sub-categories">
                <div class="section-card">
                    <div class="section-header">
                        <div class="section-header-left">
                            <div class="section-icon"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 16 12 14 15 10 15 8 12 2 12"/><path d="M5.45 5.11L2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg></div>
                            <h3 class="section-title">{{ __('admin.sub_categories') }}</h3>
                        </div>
                        <div class="d-flex align-items-center gap-2">
                            <span class="section-count">{{ $category->subCategories->count() }}</span>
                            @can('sub_categories.create')
                            <a href="{{ route('admin.categories.sub-categories.create', $category) }}" class="btn-add-section">
                    ...
                    @if($category->subCategories->isEmpty())
                        <div class="empty-state">
                            <div class="empty-icon"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" stroke-width="1.5"><polyline points="22 12 16 12 14 15 10 15 8 12 2 12"/><path d="M5.45 5.11L2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/></svg></div>
                            <p>{{ __('admin.no_sub_categories') }}</p>
                        </div>
                    @else
                        <div class="table-responsive">
                            <table class="modern-table">
                                ...
                                @foreach($category->subCategories as $sub)
                                    <tr>
                                        <td dir="rtl">{{ $sub->getTranslation('name','ar') }}</td>
                                        <td>{{ $sub->getTranslation('name','en') }}</td>
                                        <td><span class="section-count">{{ $sub->dynamicFields->count() }}</span></td>
                                        <td>
                                            <div class="tbl-actions">
                                                <a href="{{ route('admin.categories.sub-categories.show', [$category, $sub]) }}" class="tbl-btn tbl-btn-view" title="{{ __('admin.show') }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                                                </a>
                                                @can('sub_categories.edit')
                                                <a href="{{ route('admin.categories.sub-categories.edit', [$category, $sub]) }}" class="tbl-btn tbl-btn-edit" title="{{ __('admin.edit') }}">
                                                ...
                                                <button class="tbl-btn tbl-btn-delete tbl-delete-btn"
                                                    data-name="{{ $sub->getTranslation('name', app()->getLocale()) }}"
                                                    data-url="{{ route('admin.categories.sub-categories.destroy', [$category, $sub]) }}"
            ...
            {{-- Dynamic Fields Tab --}}
            <div class="tab-pane" id="tab-dynamic-fields">
                <div class="section-card">
                    <div class="section-header">
                        <div class="section-header-left">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <span class="section-count">{{ $category->dynamicFields->count() }}</span>
                            @can('dynamic_fields.create')
                            <a href="{{ route('admin.categories.dynamic-fields.create', $category) }}" class="btn-add-section">
                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                                {{ __('admin.add') }}
                            </a>
                            @endcan
                    ...
                    @if($category->dynamicFields->isEmpty())
                        <div class="empty-state">
                    ...
                    @else
                        <div class="table-responsive">
                            <table class="modern-table">
                                ...
                                <tbody>
                                @foreach($category->dynamicFields as $field)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="tbl-actions">
                                                <a href="{{ route('admin.categories.dynamic-fields.show', [$category, $field]) }}" class="tbl-btn tbl-btn-view" title="{{ __('admin.show') }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                                                </a>
                                                @can('dynamic_fields.edit')
                                                <a href="{{ route('admin.categories.dynamic-fields.edit', [$category, $field]) }}" class="tbl-btn tbl-btn-edit" title="{{ __('admin.edit') }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                                </a>
                                                @endcan
                                                ...
                                                <button class="tbl-btn tbl-btn-delete tbl-delete-btn"
                                                    ...
                                                    data-url="{{ route('admin.categories.dynamic-fields.destroy', [$category, $field]) }}"
                                                    title="{{ __('admin.delete') }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>
                                                </button>
                                                @endcan
        ...
        <div class="sidebar-modern">
            <div class="info-card">
                <div class="info-card-title">
                    ...
                    <span>{{ __('admin.category_info') }}</span>
                </div>
                <ul class="info-list">
                    <li><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
                        <strong>AR:</strong> <span dir="rtl">{{ $category->getTranslation('name','ar') }}</span></li>
                    <li><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
                        <strong>EN:</strong> {{ $category->getTranslation('name','en') }}</li>
                    <li><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                        <strong>{{ __('admin.created_at') }}:</strong> {{ $category->created_at->format('Y-m-d') }}</li>
                </ul>
            </div>
        </div>
    </div>
...
// Delete handling for any delete button (including sub-categories and dynamic fields)
const deleteForm = document.getElementById('deleteForm');
document.querySelectorAll('.tbl-delete-btn, #deleteBtn').forEach(btn => {
    btn.addEventListener('click', function(e) {
        e.preventDefault();
        let itemName = this.dataset.name || this.dataset.typeName;
        Swal.fire({
            title: '{{ __('admin.confirm_delete') }}',
            text: '{{ __('admin.delete_confirm_msg') }}: ' + itemName,
            icon: 'warning',
            ...
            if (r.isConfirmed) {
                deleteForm.action = this.dataset.deleteUrl || this.dataset.url;
                deleteForm.submit();
            }
        });
    });
