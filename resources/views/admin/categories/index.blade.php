...
@section('styles')
    ...
    <style>
        ...
        /* Card stats */
        .card-stats{display:grid;grid-template-columns:1fr 1fr;gap:.5rem;padding:.7rem 1.2rem;border-top:1px solid #f5f3ff}
        ...
        .a-view{background:#eff6ff;color:#3b82f6}
        .a-view:hover{background:#3b82f6;color:white;box-shadow:0 6px 14px rgba(59,130,246,.4)}
        .a-sub{background:#f0fdf4;color:#059669}
        .a-sub:hover{background:#059669;color:white;box-shadow:0 6px 14px rgba(5,150,105,.4)}
        .a-edit{background:#fefce8;color:#ca8a04}
        .a-edit:hover{background:#ca8a04;color:white;box-shadow:0 6px 14px rgba(202,138,4,.4)}
        .a-del{background:#fef2f2;color:#dc2626}
        .a-del:hover{background:#dc2626;color:white;box-shadow:0 6px 14px rgba(220,38,38,.4)}
        ...
        .pag-wrap .prev.dis,.pag-wrap .next.dis{opacity:.5;pointer-events:none;background:#f1f5f9!important;color:#9ca3af!important;border-color:#e2e8f0!important}
        ...
        .pag-wrap ul li a,.pag-wrap ul li span{display:flex;align-items:center;justify-content:center;width:40px!important;height:40px!important;border-radius:50%!important;background:#fff!important;color:#1e293b!important;border:1px solid transparent!important;transition:all var(--t)!important;text-decoration:none!important;font-weight:600}
...
<div class="layout-top-spacing">
    ...
    <div class="page-header-modern">
        ...
        <div class="hc">
            <div class="hta">
                <div class="hicon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>
                </div>
                <div class="ht">
                    <h1>{{ __('admin.categories') }}</h1>
                    <p>{{ __('admin.categories_management') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="gridWrap">
        @include('admin.categories.partials.grid')
    </div>
</div>

<form id="bulkForm" method="POST" action="{{ route('admin.categories.bulk-destroy') }}" style="display:none">
...
btnBulk.addEventListener('click',()=>{
    ...
    Swal.fire({
        ...
        text: '{{ __('admin.selected_count') }}: '+selectedIds.length,
        icon: 'warning',
...
function cardClick(e){
    if(e.target.closest('a,button,input,.abtn,.cb')) return;
...
window.addEventListener('popstate',()=>{
    ...
    if(searchInput) searchInput.value=v; doSearch(v);
});
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    @endif
    @if($errors->any())
    Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 5000 }).fire({ icon: 'error', title: {!! json_encode($errors->first()) !!} });
    @endif
});
</script>
@endsection
