{{-- resources/views/admin/dynamic-fields/edit.blade.php --}}
@extends('layouts.app')
...
@section('styles')
    ...
    <style>
        :root { --primary-purple: #7c3aed; --purple-light: #a78bfa; --purple-dark: #5b21b6; --purple-soft: #ede9fe; --purple-glow: rgba(124,58,237,0.22); --card-radius: 24px; --transition: 0.3s cubic-bezier(0.4,0,0.2,1); }
        ...
        <div class="header-content">
            <div class="header-title-area">
                ...
                <div class="header-text">
                    <h1>{{ __('admin.edit_dynamic_field') }}</h1>
                    <p>{{ $field->getTranslation('name', app()->getLocale()) }} (ID: {{ $field->id }})</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modern-grid">
        <div class="form-card">
            ...
            <div class="form-card-body">
                <form action="{{ $ownerType === 'category' ? route('admin.categories.dynamic-fields.update', [$owner, $field]) : route('admin.categories.sub-categories.dynamic-fields.update', [$parentCategory, $owner, $field]) }}" method="POST" id="editForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="input-group-modern">
                        ...
                        <div class="input-wrapper"><span class="flag-icon">🇸🇦</span><input type="text" name="name_ar" id="name_ar" class="form-control-modern @error('name_ar') is-invalid @enderror" value="{{ old('name_ar', $field->getTranslation('name', 'ar')) }}" placeholder="{{ __('admin.placeholder_ar') }}" required dir="rtl"></div>
                    ...
                    <div class="input-group-modern">
                        ...
                        <div class="input-wrapper"><span class="flag-icon">🇬🇧</span><input type="text" name="name_en" id="name_en" class="form-control-modern @error('name_en') is-invalid @enderror" value="{{ old('name_en', $field->getTranslation('name', 'en')) }}" placeholder="{{ __('admin.placeholder_en') }}" required dir="ltr"></div>
                    ...
                    <div class="input-group-modern">
                        ...
                        <select name="type" id="type" class="form-control-modern" style="padding:0.85rem 1rem;" required>
                            <option value="text" {{ old('type', $field->type) == 'text' ? 'selected' : '' }}>{{ __('admin.text') }}</option>
                            <option value="number" {{ old('type', $field->type) == 'number' ? 'selected' : '' }}>{{ __('admin.number') }}</option>
                            <option value="select" {{ old('type', $field->type) == 'select' ? 'selected' : '' }}>{{ __('admin.select') }}</option>
                            <option value="textarea" {{ old('type', $field->type) == 'textarea' ? 'selected' : '' }}>{{ __('admin.textarea') }}</option>
                            <option value="date" {{ old('type', $field->type) == 'date' ? 'selected' : '' }}>{{ __('admin.date') }}</option>
                        </select>
                    </div>
                    ...
                    <div class="input-group-modern">
                        ...
                        <div class="toggle-group">
                            ...
                            <label class="toggle-switch"><input type="checkbox" name="is_required" value="1" {{ old('is_required', $field->is_required) ? 'checked' : '' }}><span class="toggle-slider"></span></label>
                        </div>
                    </div>

                    <div class="form-actions">
                        <button type="submit" class="btn-primary-modern" id="updateBtn"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>{{ __('admin.update') }}</button>
        ...
        <div class="sidebar-modern">
            <div class="live-preview-card">
                ...
                <div class="preview-item"><div class="preview-item-icon"><span>🏷️</span></div><div class="preview-item-content"><div class="preview-item-label">{{ __('admin.name_ar') }}</div><div class="preview-item-value" id="previewAr">{{ $field->getTranslation('name','ar') }}</div></div></div>
                <div class="preview-item"><div class="preview-item-icon"><span>🏷️</span></div><div class="preview-item-content"><div class="preview-item-label">{{ __('admin.name_en') }}</div><div class="preview-item-value" id="previewEn">{{ $field->getTranslation('name','en') }}</div></div></div>
                <div class="preview-item"><div class="preview-item-icon"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/></svg></div><div class="preview-item-content"><div class="preview-item-label">{{ __('admin.field_type') }}</div><div class="preview-item-value" id="previewType">{{ __('admin.' . $field->type) }}</div></div></div>
            </div>
            <div class="info-card">
...
function updatePreview() {
    ...
    previewType.textContent = selectedOption ? selectedOption.text : '—';
}
...
// ========== تحميل الخيارات الحالية من قاعدة البيانات (بدون key) ==========
@php
    $existingOptions = [];
    if ($field->type === 'select' && !empty($field->options)) {
        $decoded = is_string($field->options) ? json_decode($field->options, true) : $field->options;
        if (is_array($decoded)) {
...
@if($field->type === 'select' && count($existingOptions) > 0)
    @foreach($existingOptions as $opt)
        addOptionRow(
            '{{ addslashes($opt['label']['ar'] ?? '') }}',
            '{{ addslashes($opt['label']['en'] ?? '') }}'
        );
    @endforeach
@endif

// إظهار قسم الخيارات إذا كان النوع select، وإضافة صف فارغ فقط إذا لم توجد أي خيارات
